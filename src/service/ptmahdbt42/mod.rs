...
pub(self) const CRLF: &[u8; 2] = &[0x0D, 0x0A];
pub(self) const LFLF: &[u8; 2] = &[0x0A, 0x0A];
...
impl ToString for BeamerCommand {
    fn to_string(&self) -> String {
        match self {
            BeamerCommand::PowerOn => CMD_BEAMER_PON.to_string(),
            BeamerCommand::PowerOff => CMD_BEAMER_POF.to_string(),
        }
    }
}
...
pub(crate) async fn execute_request(request: &Request) -> Result<Response> {
    ...
    stream.write(&request_buf).await?;
